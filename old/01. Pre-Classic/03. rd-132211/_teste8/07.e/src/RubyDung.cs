...
public class RubyDung : GameWindow {
    ...
    protected override void OnLoad() {
        ...
        textureAtlas = new TextureAtlas(atlasSize, atlasSize); // Dimensões do atlas (ajuste conforme necessário)

        int x = 0, y = 0;
        int rowHeight = textureSize;
        ...
        foreach(var filePath in textureFiles) {
            using(var stream = File.OpenRead(filePath)) {
                ...
                if(x + textureWidth > atlasSize) // Verifica se há espaço suficiente na linha atual
                {
                    x = 0;
                    y += rowHeight;
                    rowHeight = textureSize;
                    if(y + textureHeight > atlasSize) // Verifica se há espaço suficiente na coluna
                    {
    ...
    private int CalculateAtlasSize(int textureCount) {
        // Cada textura é considerada 16x16, ajuste conforme necessário
        int texturesPerSide = (int)Math.Ceiling(Math.Sqrt(textureCount));
        int powerOfTwo = 1;
        while(powerOfTwo < texturesPerSide) {
            powerOfTwo *= 2;
        }
        return powerOfTwo * textureSize; // Cada célula tem 16 pixels de lado
    }
