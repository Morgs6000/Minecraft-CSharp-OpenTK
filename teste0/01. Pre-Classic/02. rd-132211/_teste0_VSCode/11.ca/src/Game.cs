...
public class Highlight {
    ...
    public (Vector3, int)? Raycast(Player player, Level level, float maxDistance) {
        ...
        for(float distance = 0; distance < maxDistance; distance += step) {
            ...
            if(level.IsTile(x, y, z)) {
                // Calcular a diferença entre a posição da câmera e o bloco
                Vector3 blockCenter = new Vector3(x + 0.5f, y + 0.5f, z + 0.5f);
                Vector3 cameraToBlock = Vector3.Normalize(blockCenter - rayOrigin);

                // Calcular a face mais alinhada com a direção da câmera
                Vector3[] normals = {
                    new Vector3(0, 0, 1),  // Frente (Z+)
                    new Vector3(0, 0, -1), // Trás (Z-)
                    new Vector3(1, 0, 0),  // Direita (X+)
                    new Vector3(-1, 0, 0), // Esquerda (X-)
                    new Vector3(0, 1, 0),  // Topo (Y+)
                    new Vector3(0, -1, 0)   // Base (Y-)
                };
                ...
                for (int i = 0; i < normals.Length; i++) {
                    float dot = Vector3.Dot(cameraToBlock, normals[i]);
                    if (dot > maxDot) {
                        maxDot = dot;
                        faceIndex = i;
                    }
                ...
                Console.WriteLine($"Bloco: {x}, {y}, {z}; Face: {faceIndex}");
                return (new Vector3(x, y, z), faceIndex);
            }
        }
    ...
    public void A() {
        var result = Raycast(player, level, 5.0f);

        if(result.HasValue) {
            var(blocksPos, faceIndex) = result.Value;
            int x = (int)blocksPos.X;
            int y = (int)blocksPos.Y;
            int z = (int)blocksPos.Z;

            B(x, y, z, faceIndex);
        }
    }

    public void B(int x, int y, int z, int faceIndex) {
        shader.OnRenderFrame();
        ...
        t.Init();
        //for(int i = 0; i < 6; i++) {
            Tile.rock.RenderFace(t, x, y, z, faceIndex);
        //}
        t.OnLoad();
