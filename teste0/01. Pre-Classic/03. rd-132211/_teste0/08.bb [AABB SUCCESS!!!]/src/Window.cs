...
public class Window : GameWindow {
    ...
    private void CheckCollision() {
        /*
        Console.WriteLine($"posição do jogador: {eye.X}, {eye.Y}, {eye.Z}");

        float w = 0.3f;
        float h = 0.9f;

        if(eye.X + w > blockPos.X && eye.X - w < blockPos.X) {
            Console.WriteLine("O jogador colidiu com a face x0 do bloco");
        }
        if(eye.X - w < blockPos.X + 1 && eye.X + w > blockPos.X + 1) {
            Console.WriteLine("O jogador colidiu com a face x1 do bloco");
        }
        if(eye.Y + h > blockPos.Y && eye.Y - h > blockPos.Y) {
            Console.WriteLine("O jogador colidiu com a face y0 do bloco");
        }
        if(eye.Y - h < blockPos.Y + 1 && eye.Y + h < blockPos.Y + 1) {
            Console.WriteLine("O jogador colidiu com a face y1 do bloco");
        }
        if(eye.Z + w > blockPos.Z && eye.Z - w > blockPos.Z) {
            Console.WriteLine("O jogador colidiu com a face z0 do bloco");
        }
        if(eye.Z - w < blockPos.Z + 1 && eye.Z + w < blockPos.Z + 1) {
            Console.WriteLine("O jogador colidiu com a face z1 do bloco");
        }
        //*/
        //*
        ...
        // Limites do bloco (AABB)
        Vector3 blockMin = new Vector3(blockPos.X, blockPos.Y, blockPos.Z); // Canto mínimo do bloco
        Vector3 blockMax = new Vector3(blockPos.X + 1, blockPos.Y + 1, blockPos.Z + 1); // Canto máximo do bloco
        ...
        if(collisionX && collisionY && collisionZ) {
            ...
            Console.WriteLine($"Posição do jogador: {eye.X}, {eye.Y}, {eye.Z}");
            Console.WriteLine($"Posição do bloco: {blockPos.X}, {blockPos.Y}, {blockPos.Z}");
