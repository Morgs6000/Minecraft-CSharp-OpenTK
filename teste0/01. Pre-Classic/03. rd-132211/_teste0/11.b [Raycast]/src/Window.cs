...
public class Window : GameWindow {
    ...
    /* ..:: Raycast ::.. */
    private void Raycast(float maxDistance) {
        Vector3 rayOrigin = eye;
        Vector3 rayDirection = target;
        ...
        for(float distance = 0; distance < maxDistance; distance += stepSize) {
            ...
            if(IsTile(x, y, z)) {
                byte blockType = blocks[(y * depth + z) * width + x];
                ...
                Console.WriteLine($"Bloco encontrado na posição: ({x}, {y}, {z}), Tipo: {blockName}, Face: {face}");
    ...
    public void LoadTile(int x, int y, int z, int tex) {
