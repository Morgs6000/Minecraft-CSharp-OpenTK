...
public class AABB {
    ...
    private void PlayerPos() {
        ...
        x0 = player.playerPos.X - playerWidth;
        y0 = player.playerPos.Y - playerHeight;
        z0 = player.playerPos.Z - playerWidth;

        x1 = player.playerPos.X + playerWidth;
        y1 = player.playerPos.Y + playerHeight;
        z1 = player.playerPos.Z + playerWidth;
    }
    ...
    private void ClipCollide() {
        ...
        if(Intersects()) {
            Console.WriteLine($"Posição do jogador: {player.playerPos.ToString("F1")}");
    ...
    private void ClipXCollide() {
        ...
        if(x1 > blockPos.X && x0 < blockPos.X) {
            ...
            player.playerPos = Vector3.UnitX * (blockPos.X - playerWidth);  // Resposta à colisão
        }
        else if(x0 < (blockPos.X + 1) && x1 > (blockPos.X + 1)) {
            Console.WriteLine("Colisão com a face x1 (direita) do bloco.");
            player.playerPos = Vector3.UnitX * ((blockPos.X + 1) + playerWidth); // Resposta à colisão
        }
    }
    ...
    private void ClipYCollide() {
        ...
        if(y1 > blockPos.Y && y0 < blockPos.Y) {
            ...
            player.playerPos = Vector3.UnitY * (blockPos.Y - playerHeight); // Resposta à colisão
        }
        else if(y0 < (blockPos.Y + 1) && y1 > (blockPos.Y + 1)) {
            Console.WriteLine("Colisão com a face y1 (superior) do bloco.");
            player.playerPos = Vector3.UnitY * ((blockPos.Y + 1) + playerHeight); // Resposta à colisão
        }
    }
    ...
    private void ClipZCollide() {
        ...
        if(z1 > blockPos.Z && z0 < blockPos.Z) {
            ...
            player.playerPos = Vector3.UnitZ * (blockPos.Z - playerWidth); // Resposta à colisão
        }
        else if(z0 < (blockPos.Z + 1) && z1 > (blockPos.Z + 1)) {
            Console.WriteLine("Colisão com a face z1 (frontal) do bloco.");
            player.playerPos = Vector3.UnitZ * ((blockPos.Z + 1) + playerWidth); // Resposta à colisão
        }
    }
}
...
public class Player {
    ...
    public Vector3 playerPos {
        ...
        set {
            eye.X = value.X > 0 ? value.X : eye.X;
            eye.Y = value.Y > 0 ? value.Y : eye.Y;
            eye.Z = value.Z > 0 ? value.Z : eye.Z;
        }
    }
