...
public class Game : GameWindow {
    ...
    protected override void OnRenderFrame(FrameEventArgs args) {
        ...
        view *= Matrix4.CreateTranslation(0.0f, ((1.8f / 2) - 1.62f), 0.0f);
...
public class AABB {
    ...
    public bool Intersects(AABB other) {
        if(x0 < other.x1 && x1 > other.x0) {
            if(y0 < other.y1 && y1 > other.y0) {
                if(z0 < other.z1 && z1 > other.z0) {
                    Console.WriteLine("Esta colidino");
        ...
        Console.WriteLine("Não esta colidino");
            ...
            for(int y = 0; y < height; y++) {
                for(int z = 0; z < depth; z++) {
                    if(IsSolidTile(x, y, z)) {
                        cubes.Add(new AABB(x, y, z, x + 1, y + 1, z + 1));
                    }
                }
            }
        }
...
public class Player {
    ...
    public void Move() {
        List<AABB> cubes = level.GetCubes();

        for(int i = 0; i < cubes.Count; i++) {
            cubes[i].Intersects(aabb);
        }
    }
